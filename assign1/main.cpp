...
void parse_csv(std::string filename, std::vector<Course>& courses) {
  ...
  if (ifs.is_open()){
    ...
    char delim = ',';
    Course minicourse;
    minicourse.title = split (line, delim) [0];
    minicourse.number_of_units = split (line,delim)[1];
    minicourse.quarter = split (line, delim)[2];

    courses.push_back(minicourse);
...
void write_courses_offered(std::vector<Course>& all_courses) {
  ...
  if (!ofs.is_open()){
    ...
    std:: cerr << "Error: can't open the file" << COURSES_OFFERED_PATH << std::endl;
    return;
  ...
  for ( auto it = all_courses.begin(); it != all_courses.end(); ++it){
    ...
    if (it -> quarter != "null"){
      ...
      ofs << it -> title << ",";
      ofs << it -> number_of_units << ",";
      ofs << it -> quarter <<std::endl;
  ...
  //define a vector of offered courses, then use function to delete all the elements of this vector from the all_courses vector.
  //it's a very interesting facility of the for loop.
...
void write_courses_not_offered(std::vector<Course>& unlisted_courses) {
  ...
  if (!ofs.is_open()){
    ...
    std::cerr << "Error: can't open the file" << COURSES_NOT_OFFERED_PATH << std::endl;
    return;
